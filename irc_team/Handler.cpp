    ...
    for (int i = 0; i < evt; ++i) {
      if (_monitor[i].flags & EV_EOF) {
        ...
        if (_monitor[i].ident == _server.getServerSocket()) {
          ...
          std::pair<int, std::vector<std::string> > parsed_data =
              parseData(_msgMap[_monitor[i].ident].second);
...
void Handler::makeProtocol(int fd) {
  size_t delimiter;
  while ((delimiter = std::min(_msgMap[fd].first.find('\r'),
                               _msgMap[fd].first.find('\n'))) !=
         std::string::npos) {
...
void Handler::figureCommand(int fd,
