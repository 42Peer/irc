...
std::pair<int, std::vector<std::string>> parseData(std::string buf) {
  ...
  if (buf == "" || buf[0] == ' ') {
    ...
    if (cmd_type == PRIVMSG || cmd_type == NOTICE || cmd_type == QUIT ||
        cmd_type == KICK)
    ...
    else if ((cmd_type == JOIN || cmd_type == NICK || cmd_type == PART ||
              cmd_type == PASS) &&
             ret_vector.size() != 1)
      deleteExceededArgs(ret_vector, 1, cmd_type);
    else if (cmd_type == KICK &&
             (ret_vector.size() != 2 && ret_vector.size() != 3))
...
int splitMessageCommand(int ctype, std::string data,
